...
import org.zcashcoinj.utils.ContextPropagatingThreadFactory;
 ...
 * zcashcoinj for a specific network. You can get an instance of this class through calling {@link #get()}.</p>
 *
...
public class Context {
    ...
    public Context(NetworkParameters params, int eventHorizon, Coin feePerKb, boolean ensureMinRequiredFee) {
        log.info("Creating zcashcoinj {} context.", VersionMessage.BITCOINJ_VERSION);
     ...
     * independently use zcashcoinj (or possibly alt coin forks of it) can operate correctly.
     *
    ...
    public static Context get() {
        ...
        if (tls == null) {
            if (isStrictMode) {
                log.error("Thread is missing a zcashcoinj context.");
                log.error("You should use Context.propagate() or a ContextPropagatingThreadFactory.");
                throw new IllegalStateException("missing context");
            }
            if (lastConstructed == null)
                throw new IllegalStateException("You must construct a Context object before using zcashcoinj!");
            slot.set(lastConstructed);
            log.error("Performing thread fixup: you are accessing zcashcoinj via a thread that has not had any context set on it.");
